...
def train(config='conf/config.yaml', **kwargs):
    ...
    # reset selected module after loading model_init
    if configs.get('reset_layer4_se_res2block', False):
        if rank == 0:
            logger.info("<== Reset layer4.se_res2block ==>")
            logger.info("Re-initialize layer4.se_res2block after loading model_init")

        def reset_module(m):
            if hasattr(m, 'reset_parameters'):
                m.reset_parameters()

        model.layer4.se_res2block.apply(reset_module)
    ...
    # fine-tune setting: freeze all parameters except selected modules
    if configs.get('freeze_except_layer4_se', False):
        trainable_prefixes = configs.get(
            'trainable_prefixes',
            ['layer4.se_res2block', 'projection']
        )
        ...
        if rank == 0:
            logger.info("<== Fine-tuning Freeze Setting ==>")
            logger.info("Only parameters with these prefixes are trainable: {}".format(
                trainable_prefixes
            ))
            ...
            for name, param in model.named_parameters():
                ...
                if param.requires_grad:
                    ...
                    logger.info("trainable: {} {}".format(name, tuple(param.shape)))

            logger.info("trainable params: {}".format(trainable_num))
            logger.info("total params: {}".format(total_num))
    ...
    trainable_parameters = filter(lambda p: p.requires_grad, ddp_model.parameters())
    optimizer = getattr(torch.optim,
                        configs['optimizer'])(trainable_parameters,
